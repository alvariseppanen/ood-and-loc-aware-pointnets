...
def from_prediction_to_label_format_dbot(center, angle_class, angle_res,\
                                    size_class, size_res):
    ''' Convert predicted box parameters to label format. '''
    l,w,h = class2size(size_class, size_res)
    ry = class2angle(angle_class, angle_res, NUM_HEADING_BIN)
    tz,tx,ty = center
    return h,w,l,-tx,-ty+h/2,tz,-ry #np.pi/2-ry

def write_detection_results_dbot(result_dir, id_list, type_list, box2d_list, center_list, \
    ...
    for i in range(len(center_list)):
        ...
        h,w,l,tx,ty,tz,ry = from_prediction_to_label_format_dbot(center_list[i],
...
def from_prediction_to_label_format_lidar(center, angle_class, angle_res,\
    ...
    ''' Convert predicted box parameters to label format. '''
    l,w,h = class2size(size_class, size_res)
    ry = class2angle(angle_class, angle_res, NUM_HEADING_BIN)+np.pi/2
    tx,ty,tz = center
...
def from_prediction_to_label_format_cam(center, angle_class, angle_res,\
                                    size_class, size_res, V2C, R0, rotation_angle):
    ''' Convert predicted box parameters to label format. '''
    l,w,h = class2size(size_class, size_res)
    #ry = np.pi/2 - class2angle(angle_class, angle_res, NUM_HEADING_BIN) 
    ry = -class2angle(angle_class, angle_res, NUM_HEADING_BIN) #- rotation_angle
...
def write_detection_results_cam(result_dir, id_list, type_list, box2d_list, center_list, \
                            heading_cls_list, heading_res_list, \
                            size_cls_list, size_res_list, score_list, lidar2cam, R0, calib_file, rotation_angles):
    ''' Write results to KITTI format label files. '''
    if result_dir is None: return
    results = {} # map from idx to list of strings, each string is a line (without \n)
    ...
    for i in range(len(center_list)):
        ...
        h,w,l,tx,ty,tz,ry = from_prediction_to_label_format_cam(center_list[i],
            heading_cls_list[i], heading_res_list[i],
            size_cls_list[i], size_res_list[i], lidar2cam, R0, rotation_angles[i])
