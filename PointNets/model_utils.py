...
g_type_mean_size2 = torch.tensor([[3.88311640418,1.62856739989,1.52563191462], [1.76282397,0.59706367,1.73698127], [0.84422524,0.66068622,1.76255119]]).cuda()
# car, cyc, ped
...
def get_box3d_corners_helper(centers, headings, sizes):
    ...
    corners_3d = torch.bmm(R.double(), corners.double()) # (N,3,8)
...
class BoxNetLoss(nn.Module):
    def __init__(self):
        super(BoxNetLoss, self).__init__()
    ...
    def forward(self, \
        ...
        #center_dist = torch.norm(center_label-center,dim=1)#(32,)
        center_loss = huber_loss(center_dist, delta=2.0)

        stage1_center_dist = torch.norm(center-stage1_center,dim=1)#(32,)
        #stage1_center_dist = torch.norm(center_label-stage1_center,dim=1)#(32,)
        ...
        hcls_onehot = torch.eye(NUM_HEADING_BIN)[heading_class_label.long()].cuda()  # 32,12 # eye = 12 identity matrix
