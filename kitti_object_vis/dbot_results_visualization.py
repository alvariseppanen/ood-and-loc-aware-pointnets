...
def draw_lidar(
    ...
    mlab.view(azimuth=180, elevation=70, focalpoint=[0.0, 0.0, 0.0], distance=32.0, figure=fig)
    return fig
...
def show_lidar_with_boxes(
    ...
    if objects is not None:
        ...
        for obj in objects:
            ...
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=color, text_scale=(0.4,0.4,0.4) ,label=str(obj.type))
    ...
    if objects_pred is not None:
        ...
        for obj in objects_pred:
            ...
            print("box3d_pts_3d_velo:")
            print(box3d_pts_3d_velo)
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=color, text_scale=(0.4,0.4,0.4) ,label=str(obj.type) + ' ' + str(obj.score)[:4])
            # Draw heading arrow
            '''_, ori3d_pts_3d = kitti_util.compute_orientation_3d(obj, calib.P)
            ori3d_pts_3d_velo = calib.project_rect_to_velo(ori3d_pts_3d)
            x1, y1, z1 = ori3d_pts_3d_velo[0, :]
...
if __name__ == "__main__":
    ...
    dataset = 'car'
    dataset = 'pedestrian'
    lidar_data = sorted(glob.glob('/home/alvari/test_ws/dbot_dataset/' + dataset + '_dataset/velodyne/*.bin'), key=key_func)
    calib_data = sorted(glob.glob('/home/alvari/test_ws/dbot_dataset/' + dataset + '_dataset/calib/*.txt'), key=key_func)
    label_data = sorted(glob.glob('/home/alvari/test_ws/dbot_dataset/' + dataset + '_dataset/labels/*.txt'), key=key_func)
    pred_data = sorted(glob.glob('/home/alvari/test_ws/dbot_dataset/' + dataset + '_dataset/pred_lidar/*.txt'), key=key_func)
    ...
    for i in range(len(pred_data)):
        ...
        Scan.open_scan(lidar_data[i]) # 20
        xyz_list = Scan.points
        #fig = draw_lidar(xyz_list)

        pc_velo = xyz_list

        calibration = kitti_util.Calibration(calib_data[1])
        labels = kitti_util.read_label(label_data[i])
        pred_obj = kitti_util.read_label(pred_data[i])

        fig = show_lidar_with_boxes(pc_velo, objects=labels, calib=calibration, img_fov=False, objects_pred=pred_obj, depth=None, cam_img=None)

        #mlab.savefig('/home/alvari/test_ws/dbot_dataset/' + dataset + '_dataset/imgs/pc_view{}.jpg'.format(i), figure=fig)
        #mlab.close(all=True)
        raw_input()
