...
const int32_t MAX_OCCLUSION[3]  = {0, 0, 0};        // maximum occlusion level of the groundtruth used for evaluation
const double  MAX_TRUNCATION[3] = {0.0, 0.0, 0.0}; // maximum truncation level of the groundtruth used for evaluation
...
//const double MIN_OVERLAP[3][3] = {{0.5, 0.5, 0.5}, {0.5, 0.5, 0.5}, {0.25, 0.5, 0.5}};
const double MIN_OVERLAP[3][3] = {{0.5, 0.25, 0.25}, {0.5, 0.25, 0.25}, {0.25, 0.25, 0.25}};
...
vector<tGroundtruth> loadGroundtruth(string file_name,bool &success) {
  ...
  while (!feof(fp)) {
    tGroundtruth g;
    char str[255];
    double occ = 0.0; // 1, 3, 2
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                   str, &g.truncation, &occ, &g.box.alpha, &g.box.x1, &g.box.y1,
                   &g.box.x2, &g.box.y2, &g.h, &g.w, &g.l, &g.t1, &g.t2, &g.t3, 
                   &g.ry )==15) {
      g.box.type = str;
      g.box.x1 = 0;
      g.box.y1 = 0;
